...
export default class Registbox extends Component {
  ...
  usertipsdel(e) {
    ...
    if (e.target.value.length < 2 || e.target.value.length > 8) {
      ...
      axios.post('http://localhost:7001/checkUser', {
  ...
  phonetipsdel(e) {
    ...
    if (e.target.value.length != 11) {
      ...
      //验证手机号是否存在
      axios.post('http://localhost:7001/checkPhone', {
        ...
        if (res.data.length == 1) {
          ...
          this.setState({ phonein: e.target.value });
          console.log(this.state.phonein)
  ...
  render() {
    return (
      <>
        <div>用户名:</div>
        <Input className="userinput" size="large" placeholder="请输入用户名" prefix={<UserOutlined />} style={{ marginTop: "10px" }} onFocus={this.userhandle.bind(this)} onBlur={this.usertipsdel.bind(this)} />
